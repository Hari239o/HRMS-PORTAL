  ...
  try {
    ...
    const formatted = await Promise.all(leaves.map(async l => {
      ...
      return {
        ...
        employee: { name: emp.name || 'Unknown', department: emp.department || 'Unknown', empId: emp.empId || 'UNKN', avatar: emp.avatar ? await generateSignedUrl(emp.avatar, 60 * 24 * 7) : null }
      };
    }));
    res.json(formatted);
  } catch (error) {
