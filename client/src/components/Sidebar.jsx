...
const Sidebar = ({ isOpen, setIsOpen }) => {
 ...
 <aside className={`fixed md:relative inset-y-0 left-0 z-[60] w-72 h-full bg-white border-r border-slate-200 flex flex-col shadow-sm transition-transform ${isOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
 ...
 {user.role === 'student' && (
 <div className="flex bg-white/40 border border-white/60 p-1 rounded-xl overflow-hidden shadow-inner mt-2 mx-4">
 <button 
 onClick={() => setPortalMode('hr')}
 className={`flex-1 py-1.5 text-xs font-bold rounded-lg ${!isStudent ? 'bg-white text-[#ff5a1f] shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
 >
 HR Portal
 </button>
 <button 
 onClick={() => setPortalMode('student')}
 className={`flex-1 py-1.5 text-xs font-bold rounded-lg ${isStudent ? 'bg-sky-500 text-white shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
 >
 Student
 </button>
 </div>
 ...
 key={item.path}
 href={item.path}
 className={`flex items-center gap-3 px-4 py-3 rounded-2xl group ${getThemeClasses(isActive)}`}
 >
 <item.icon size={20} className="transition-transform group-" />
 ...
 <Link 
 href="/settings"
 className={`flex-1 p-2 rounded-xl flex items-center justify-center border ${pathname === '/settings' ? 'bg-[#ff5a1f]/10 text-[#ff5a1f] border-[#ff5a1f]/20' : 'bg-white text-slate-500 hover:text-[#ff5a1f] hover:shadow-sm border-slate-200'}`}
 title="Settings"
 >
 <Settings size={18} />
 </Link>
 ) : (
 <button className={`flex-1 p-2 bg-white rounded-xl text-slate-500 hover:text-[#ff5a1f] hover:shadow-sm flex items-center justify-center border border-slate-200`} title="Settings">
 <Settings size={18} />
 </button>
 )}
 <button className={`flex-1 p-2 bg-white rounded-xl text-slate-500 hover:text-[#ff5a1f] hover:shadow-sm flex items-center justify-center border border-slate-200`} title="Notifications">
 <Bell size={18} />
 </button>
 </div>
 ...
 <button
 onClick={logout}
 className="w-full py-2.5 bg-rose-50 rounded-xl text-rose-600 hover:bg-rose-100 hover:shadow-sm flex items-center justify-center gap-2 border border-rose-100 font-bold text-sm"
 >
