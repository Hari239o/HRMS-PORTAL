...
export default function Documents() {
 ...
 <div className="relative rounded-3xl shadow-xl border border-white/40 overflow-hidden bg-gradient-to-br from-white/80 to-blue-50/40 p-6">
 ...
 <div 
 className={`h-full rounded-full shadow-sm ${progress === 100 ? 'bg-gradient-to-r from-emerald-400 to-emerald-500' : 'bg-gradient-to-r from-amber-400 to-amber-500'}`}
 style={{ width: `${progress}%` }}
 ></div>
 </div>
 {progress < 100 && (
 <p className="text-xs text-amber-700 mt-4 flex items-center gap-1.5 font-bold bg-amber-100/50 border border-amber-200 p-3 rounded-xl w-max">
 ...
 return (
 <div key={doc.id} className={`relative rounded-3xl p-6 border shadow-lg hover:shadow-2xl group ${isUploaded ? 'bg-gradient-to-br from-white/80 to-emerald-50/40 border-emerald-100/50' : 'bg-white/60 border-slate-200/50 hover:border-blue-300'}`}>
 ...
 {isUploaded ? (
 <span className="bg-emerald-100/80 text-emerald-700 text-[10px] font-black px-3 py-1.5 rounded-xl flex items-center gap-1.5 uppercase tracking-widest border border-emerald-200 shadow-sm">
 <CheckCircle size={14} /> Uploaded
 </span>
 ) : (
 <span className="bg-slate-100/80 text-slate-500 text-[10px] font-black px-3 py-1.5 rounded-xl uppercase tracking-widest border border-slate-200 shadow-sm">
 Required
 </span>
 )}
 </div>
 ...
 className="flex-1 text-center py-3 bg-white/80 hover:bg-blue-600 text-blue-600 hover:text-white font-black text-xs uppercase tracking-widest rounded-xl border border-blue-100 shadow-sm flex justify-center items-center gap-2 group-hover:border-blue-200"
 >
 <File size={16} /> View
 </a>
 <button 
 ...
 className="flex-1 py-3 bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-600 hover:to-amber-700 text-white font-black text-xs uppercase tracking-widest rounded-xl shadow-lg shadow-amber-500/30 flex justify-center items-center gap-2 "
 ...
 onClick={() => handleUploadClick(doc.id)}
 disabled={isUploadingThis}
 className="w-full py-3 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white font-black text-xs uppercase tracking-widest rounded-xl shadow-lg shadow-blue-500/30 flex justify-center items-center gap-2 "
 >
 {isUploadingThis ? (
 <>Uploading...</>
 ) : (
