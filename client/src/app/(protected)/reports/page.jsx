    ...
    <div className="space-y-8 fade-in">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <button 
          ...
          className="px-6 py-3 bg-slate-900 text-white rounded-2xl font-black text-sm hover:bg-slate-800 shadow-xl shadow-slate-200 transition-all active:scale-95 flex items-center gap-2"
        >
                      ...
                      <td className="px-8 py-6 text-right">
                        <div className="inline-flex flex-col items-end">
                          ...
                          <div className="w-32 bg-slate-100 h-1.5 rounded-full overflow-hidden">
                            <div 
                              className={`h-full transition-all duration-1000 ${parseFloat(rate) > 80 ? 'bg-emerald-500' : 'bg-amber-500'}`}
          ...
          <div className="relative w-full max-w-5xl rounded-[32px] shadow-2xl overflow-hidden flex flex-col bg-white/70 backdrop-blur-2xl border border-white/60 animate-in zoom-in-95 duration-300 my-auto h-fit max-h-[95vh]">
            ...
            <div className="p-4 md:p-8 border-b border-white/40 flex justify-between items-start bg-white/30 backdrop-blur-md shrink-0">
              <div className="flex items-center gap-4 md:gap-5">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-3 mt-2">
                    <span className="px-3 py-1 bg-white/60 backdrop-blur-md text-slate-700 border border-white/50 rounded-lg text-[10px] font-black uppercase tracking-widest shadow-sm">{selectedEmp.department}</span>
              ...
              <button 
                ...
                className="p-3 bg-white/50 backdrop-blur-md border border-white/50 text-slate-500 hover:text-slate-800 rounded-xl shadow-sm hover:shadow-md transition-all hover:-translate-y-0.5"
              >
                <X size={20} />
              </button>
            </div>
                ...
                <div className="space-y-8">
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    <div>
                      ...
                      <div className="bg-white/50 backdrop-blur-xl rounded-3xl p-6 shadow-sm border border-white/60 h-[400px] overflow-y-auto custom-scrollbar">
                        {(() => {
                          const activities = [];
                        ...
                        {empPerformance?.submissions?.length > 0 ? (
                          <div className="space-y-4">
                            {empPerformance.submissions.sort((a,b) => new Date(b.date) - new Date(a.date)).map((sub, idx) => (
                              <div key={idx} className="bg-white/70 backdrop-blur-md p-4 rounded-2xl shadow-sm border border-white/60 hover:-translate-y-1 transition-transform group">
