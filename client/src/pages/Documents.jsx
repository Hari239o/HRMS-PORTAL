...
export default function Documents() {
  ...
  if (user.role === 'admin') {
    ...
    const empDocs = selectedEmp ? (selectedEmp.documents || {}) : {};
    const completedCount = REQUIRED_DOCS.filter(doc => empDocs[doc.id]).length;
          ...
          <>
            <div className="relative rounded-3xl shadow-xl border border-white/40 overflow-hidden bg-gradient-to-br from-white/80 to-blue-50/40 backdrop-blur-xl p-6">
              <div className="flex justify-between items-center mb-2">
                <h3 className="font-black text-slate-800 flex items-center gap-2 text-lg">
                  ...
                  {selectedEmp.name}'s Profile Completion
                </h3>
              ...
              {progress < 100 && (
                <p className="text-xs text-amber-700 mt-4 flex items-center gap-1.5 font-bold bg-amber-100/50 backdrop-blur-sm border border-amber-200 p-3 rounded-xl w-max">
                  <AlertCircle size={16} className="text-amber-600" /> Missing {REQUIRED_DOCS.length - completedCount} required document(s)
                </p>
              )}
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
              {REQUIRED_DOCS.map((doc) => {
                const isUploaded = !!empDocs[doc.id];
                  ...
                  <div key={doc.id} className={`relative rounded-3xl p-6 transition-all duration-300 border backdrop-blur-xl shadow-lg hover:shadow-2xl group ${isUploaded ? 'bg-gradient-to-br from-white/80 to-emerald-50/40 border-emerald-100/50' : 'bg-white/60 border-slate-200/50 hover:border-blue-300'}`}>
                    <div className="flex justify-between items-start mb-6">
                      ...
                      {isUploaded ? (
                        ...
                        <span className="bg-rose-100/80 backdrop-blur text-rose-600 text-[10px] font-black px-3 py-1.5 rounded-xl uppercase tracking-widest border border-rose-200 shadow-sm">
                          Missing
                        </span>
                      )}
                    </div>
                    ...
                    <div className="flex gap-3">
                      {isUploaded ? (
                        <>
                          ...
                          <button 
                            onClick={() => handleDownload(fileUrl, `${selectedEmp.name}_${doc.title}`)}
                            className="flex-1 py-3 bg-gradient-to-r from-emerald-500 to-emerald-600 hover:from-emerald-600 hover:to-emerald-700 text-white font-black text-xs uppercase tracking-widest rounded-xl transition-all shadow-lg shadow-emerald-500/30 flex justify-center items-center gap-2 hover:-translate-y-0.5"
                          >
                            <DownloadCloud size={16} /> Download
                          </button>
                        </>
                      ) : (
                        <div className="w-full py-3 bg-slate-100/50 backdrop-blur text-slate-400 font-bold text-xs uppercase tracking-widest rounded-xl flex justify-center items-center gap-2 border border-slate-200 border-dashed">
                ...
                {isUploaded ? (
                  <>
                    ...
                    <button 
                      ...
                      className="px-5 py-3 bg-gradient-to-r from-emerald-500 to-emerald-600 hover:from-emerald-600 hover:to-emerald-700 text-white font-black text-xs uppercase tracking-widest rounded-xl transition-all shadow-lg shadow-emerald-500/30 hover:-translate-y-0.5"
                      title="Update Document"
                    >
                      {isUploadingThis ? '...' : <UploadCloud size={16} />}
                    </button>
                  </>
                ) : (
                  <button 
